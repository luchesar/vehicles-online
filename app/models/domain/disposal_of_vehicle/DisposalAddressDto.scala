...
object DisposalAddressDto {
  ...
  def from(addressViewModel: AddressViewModel): DisposalAddressDto = {
    ...
    val trimmingRequired = checkForLinesOverMax(addressViewModel.address)
    val addressWithMandatoryLines: AddressViewModel = if (addressViewModel.address.size == 2) AddressViewModel(addressViewModel.uprn, Seq(BuildingNameOrNumberHolder) ++ addressViewModel.address)
  ...
  private def checkForLinesOverMax(sourceAddress: Seq[String]): Boolean = {
    ...
    else checkForLinesOverMax(sourceAddress.tail)
  }

  private def buildStandardDto(sourceAddress: AddressViewModel): DisposalAddressDto = {
  ...
  private def rebuildAddress(addressViewModel: AddressViewModel): DisposalAddressDto = {
