...
object DisposalOfVehicleSessionState {
  ...
  implicit class RequestAdapter[A](val request: Request[A]) extends AnyVal {
    def getCookie[B](implicit fjs: Reads[B], cacheKey: CacheKey[B], encryption: CookieEncryption): Option[B] = {
    ...
    def getCookieNamed(key: String)(implicit encryption: CookieEncryption): Option[String] = {
      val salt = CryptoHelper.getSaltFromRequest(request).getOrElse("")
  ...
  implicit class SimpleResultAdapter(val inner: SimpleResult) extends AnyVal {
    ...
    def withCookie[A](model: A)(implicit tjs: Writes[A], cacheKey: CacheKey[A], request: Request[_], encryption: CookieEncryption): SimpleResult = {
    ...
    def withCookie(key: String, value: String)(implicit request: Request[_], encryption: CookieEncryption): SimpleResult = {
  ...
  implicit class FormAdapter[A](val f: Form[A]) extends AnyVal {
    def fill()(implicit request: Request[_], fjs: Reads[A], cacheKey: CacheKey[A], encryption: CookieEncryption): Form[A] =
