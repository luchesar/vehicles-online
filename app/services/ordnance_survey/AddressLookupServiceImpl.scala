...
class AddressLookupServiceImpl @Inject() (ws: services.WebService) extends AddressLookupService {
  ...
  override def fetchAddressesForPostcode(postcode: String): Future[Seq[(String, String)]] = {
    ...
    val futureOfResponse = ws.url(endPoint).withAuth(username = username, password = password, scheme = AuthScheme.BASIC).get() // TODO should we add a .withRequestTimeout()? and if yes then what time for the timeout assuming slow connections?
