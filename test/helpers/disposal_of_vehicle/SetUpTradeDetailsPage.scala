...
object SetUpTradeDetailsPage {
  ...
  def setupCache(traderPostcode: String = postcodeValid) = {
  ...
  def happyPath(browser: TestBrowser, traderBusinessName: String = traderBusinessNameValid, traderPostcode: String = postcodeValid) = {
    browser.goTo(url)
