...
class DayMonthYearSpec extends WordSpec with Matchers {
  "DayMonthYear" should {
    "return the correct 'yyyy-MM-dd' date format" in {
      val dmy = DayMonthYearObject(1, 1, 1963)
      dmy.`yyyy-MM-dd` shouldEqual "1963-01-01"
    }

    "subtract 1 day from 26-6-2010 to give 25-6-2010" in {
      val dmy = DayMonthYearObject(26, 6, 2010)
      (dmy - 1 day) shouldEqual DayMonthYearObject(25, 6, 2010)
    }

    "subtract 6 days from 26-6-2010 to give 20-6-2010" in {
      val dmy = DayMonthYearObject(26, 6, 2010)
      (dmy - 6 days) shouldEqual DayMonthYearObject(20, 6, 2010)
      (dmy - 6 day) shouldEqual DayMonthYearObject(20, 6, 2010)
    }

    "subtract 2 weeks from 26-6-2010 to give 12-6-2010" in {
      val dmy = DayMonthYearObject(26, 6, 2010)
      (dmy - 2 weeks) shouldEqual DayMonthYearObject(12, 6, 2010)
    }

    "subtract 5 months from 26-6-2010 to give 26-1-2010" in {
      val dmy = DayMonthYearObject(26, 6, 2010)
      (dmy - 5 months) shouldEqual DayMonthYearObject(26, 1, 2010)
    }

    "subtract 6 months from 26-6-2010 to give 26-12-2009" in {
      val dmy = DayMonthYearObject(26, 6, 2010)
      (dmy - 6 months) shouldEqual DayMonthYearObject(26, 12, 2009)
    }

    "subtract 14 years from 26-6-2010 to give 26-6-1996" in {
      val dmy = DayMonthYearObject(26, 6, 2010)
      (dmy - 14 years) shouldEqual DayMonthYearObject(26, 6, 1996)
    }

    "subtract 2 days, 4 months and 1 year from 26-6-2010 to give 24-2-2009" in {
      val dmy = DayMonthYearObject(26, 6, 2010)
      (((dmy - 2 days) - 4 months) - 1 year) shouldEqual DayMonthYearObject(24, 2, 2009)
    }
    ...
    "accept a Joda DateTime" in {
      val dmy = DayMonthYearObject(new DateTime(2013, 2, 23, 0, 0))
    ...
    "format as '23 September, 2013' " in {
      val dmy = DayMonthYearObject(23, 9, 2013)
      dmy.`dd month, yyyy` shouldEqual "23 September, 2013"
    }

    "include time" in {
      val dmyWithTime = DayMonthYearObject(23, 9, 2013).withTime(hour = 14, minutes = 55)
